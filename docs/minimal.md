...
> **NOTE — DaemonCores-Phone minimal variant.**
> This document describes the **smartphone minimal variant** of the bootc/OSTree
> image: an ultra-lean Debian Trixie ARM64 image that boots to a fully supported
> CLI on Android smartphones via the Halium standard pipeline. The kernel +
> initramfs live in the Android `boot.img` (see
> [`architecture.md`](architecture.md)); the rootfs documented here is the
> bootc/OSTree userspace that the Halium initramfs mounts.
>
> The content below was carried over from the `debian-bootc` template and still
> references some desktop/SBC artefacts (x86_64 builds, SBC device-tree tables,
> `firstboot-user-setup`, U-Boot). These will be pruned as the smartphone
> minimal variant converges (Roadmap P01/P14). The ARM64 section is the
> relevant one for smartphones; the x86_64 section is retained for reference
> until the cleanup lands.
...
The Containerfile exposes four build-time `ARG`s. None are mandatory; the
...
| `KERNEL_VARIANT` | `stock`   | `stock` \| `minimal` | `stock` installs the Debian `linux-image-<arch>` metapackage. `minimal` installs the custom `linux-image-minimal` package built from `kernel/config-minimal-<arch>` by the `bootc-debs-builder` pipeline (disabled subsystems: `SND`, `DRM`, `BT`, `WLAN`, `USB_HID`). A full kernel build adds ~30-90 min to the pipeline. |
| `AUTOUPDATE`     | `1`       | `1` \| `0`       | `1` writes `/usr/lib/bootc/autoupdate` so bootc performs automated ostree upgrades (the `_autoupdate` tag variant). `0` omits the flag file, disabling auto-update for the `_lock` tag variant (minimal RAM, no background upgrade overhead). |
